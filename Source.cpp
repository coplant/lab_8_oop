...
void Container::Out_Only_Truck(ofstream& ofst) {
    ...
    for (int i = 0; i < Len; i++) {
        ofst << i << ": ";
        Temp->Cont->Out_Only_Truck(Temp->Cont->Get_Motor_power(), ofst);
...
void Car::Out_Only_Truck(int Motor_power, ofstream& ofst) {
    ofst << endl;
}
...
void Truck::Out_Only_Truck(int Motor_power, ofstream& ofst) {
    Out_Data(Motor_power, ofst);
}
